...
clock_select(void)
{
	...
	if (typesystem != NULL) {
		...
		if (debug) {
                        ...
                        /*
                        if (prev_offset != 0){
                        	if (prev_offset > (sys_offset*1000)){
                                    deflect = -1000;
                                } else {
                                    deflect = 1000;
                                } 
                        }
                        printf("%lu\t%.9f\t%.9f\n", now.tv_sec * 1000000000 + now.tv_nsec * 1, 
                                              (sys_offset*1000)+deflect,
                                              (sys_offset*1000));
                        */
                        printf("%lu\t%.9f\n", now.tv_sec * 1000000000 + now.tv_nsec * 1, 
                                              (sys_offset*1000));
			//printf("select: %llu combine offset %.9f jitter %.9f\n",
			//    current_time, sys_offset, sys_jitter);
                        float prev_offset = sys_offset*1000;
                }
#endif
	}
#ifdef REFCLOCK
